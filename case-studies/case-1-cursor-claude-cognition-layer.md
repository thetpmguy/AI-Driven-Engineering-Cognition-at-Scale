...
CASE 1: Cursor + Claude as an Engineering Cognition Layer (STAR)

#S — Situation
I was working with a multi-region e-commerce retailer operating web and mobile storefronts across NA, EMEA, and APAC. We had roughly 120 engineers split across checkout, catalog, fulfillment, identity, and platform teams. The stack was modern but dense: React/Next.js on the frontend; Node, Java, and Kotlin services on the backend; event-driven integrations; Kubernetes; Terraform; Datadog; GitHub Actions; PagerDuty.
...
Delivery metrics were slipping. PRs were bouncing back and forth for structural reasons. New hires were taking almost two weeks before making a meaningful contribution. Debugging required spelunking across logs, traces, event payloads, and configs. People felt busy but not effective.
Leadership kept hearing “we need to move faster,” but without a concrete lever to pull.

#T — Task
My task was not “introduce AI.”
My task was to reduce engineering friction without lowering quality, and to do it in a way that was provable, auditable, and safe for a regulated e-commerce environment.
That meant:
...
Measuring improvement with delivery and quality metrics, not vibes
...
Before any rollout, we captured six weeks of baseline data. This was non-negotiable, because otherwise you can’t prove impact.
We measured:
...
Ingesting architecture decision records (ADRs)
Ingesting checkout, promo, tax, and shipping runbooks
Ingesting API contracts and domain models
Ingesting incident postmortems
Creating a domain glossary to stop semantic drift (e.g., refund vs exchange vs return)
...
This step alone eliminated a huge amount of confusion in explanations.
...
The engineer highlighted files and asked a question like “how does checkout reservation work?”
...
These were not suggestions; they were structured artifacts engineers could work from.
...
This was key: the AI reviewer was not subjective. It only spoke when it could point to an ADR or policy.
Step 6: Governance and security
We shipped this with guardrails from day one:
...
PR cycle time dropped from 2.4 days to 1.5 days (≈37%)
Review iterations per PR dropped from 3.1 to 2.0
PRs with weak tests dropped from 28% to 12%
Lead time for change dropped from 6.5 days to 4.7 days
Operations:
Regression incidents dropped from 5–7/month to 3–4/month
MTTR for known issue classes (cache, promo, tax) improved by ~20–30%
Onboarding:
Time to first meaningful PR dropped from ~12 days to ~7 days
The credibility move was crucial: we never claimed “AI wrote X% of our code.”
We showed:
DORA trends
GitHub analytics
CI test quality metrics
Incident trend analysis
The story was simple and honest: we removed friction from planning, repetition, and review.
